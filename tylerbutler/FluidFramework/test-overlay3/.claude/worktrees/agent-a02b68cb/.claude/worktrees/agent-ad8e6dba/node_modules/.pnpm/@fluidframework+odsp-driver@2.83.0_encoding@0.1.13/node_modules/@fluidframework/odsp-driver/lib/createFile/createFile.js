...
import { loggerToMonitoringContext, PerformanceEvent, } from "@fluidframework/telemetry-utils/internal";
import { ClpCompliantAppHeader } from "./../contractsPublic.js";
import { createOdspUrl } from "./../createOdspUrl.js";
import { getHeadersWithAuth } from "./../getUrlAndHeadersWithAuth.js";
import { OdspDriverUrlResolver } from "./../odspDriverUrlResolver.js";
import { checkForKnownServerFarmType, getApiRoot } from "./../odspUrlHelper.js";
import { appendNavParam, buildOdspShareLinkReqParams, createCacheSnapshotKey, getWithRetryForTokenRefresh, snapshotWithLoadingGroupIdSupported, } from "./../odspUtils.js";
 ...
 */
export async function createNewFluidFile(getAuthHeader, newFileInfo, logger, createNewSummary, epochTracker, fileEntry, createNewCaching, forceAccessTokenViaAuthorizationHeader, isClpCompliantApp, enableSingleRequestForShareLinkWithCreate, resolvedUrl) {
    // Check for valid filename before the request to create file is actually made.
    if (isInvalidFileName(newFileInfo.filename)) {
        throw new NonRetryableError(
        // pre-0.58 error message: Invalid filename
        "Invalid filename for createNew", OdspErrorTypes.invalidFileNameError, { driverVersion });
    }
    let itemId;
    let pendingRename;
    let summaryHandle = "";
    ...
    if (createNewSummary === undefined) {
        const content = await createNewEmptyFluidFile(getAuthHeader, newFileInfo, logger, epochTracker);
    ...
    if (shareLinkInfo?.createLink?.link) {
        ...
        newWebUrl = appendNavParam(newWebUrl, odspResolvedUrl, odspResolvedUrl.dataStorePath ?? "/", odspResolvedUrl.codeHint?.containerPackageName);
    ...
    if (createNewSummary !== undefined && createNewCaching) {
        ...
        await epochTracker.put(createCacheSnapshotKey(odspResolvedUrl, snapshotWithLoadingGroupIdSupported(loggerToMonitoringContext(logger).config)), snapshot);
    }
 ...
 */
function extractShareLinkData(response, enableSingleRequestForShareLinkWithCreate) {
    let shareLinkInfo;
    ...
    return getWithRetryForTokenRefresh(async (options) => {
        ...
        return PerformanceEvent.timedExecAsync(logger, {
            ...
            if (!content?.id) {
                ...
                "ODSP CreateFile call returned no item ID (for empty file)", OdspErrorTypes.incorrectServerResponse, { driverVersion });
            }
            event.end({
...
export async function renameEmptyFluidFile(getAuthHeader, odspParts, requestedFileName, logger, epochTracker) {
    const initialUrl = `${getApiRoot(new URL(odspParts.siteUrl))}/drives/${odspParts.driveId}/items/${odspParts.itemId}?@name.conflictBehavior=rename`;
    return getWithRetryForTokenRefresh(async (options) => {
        const url = initialUrl;
        const method = "PATCH";
        const authHeader = await getAuthHeader({ ...options, request: { url, method } }, "renameFile");
        return PerformanceEvent.timedExecAsync(logger, { eventName: "renameFile" }, async (event) => {
            const headers = getHeadersWithAuth(authHeader);
            headers["Content-Type"] = "application/json";
            const fetchResponse = await runWithRetry(async () => epochTracker.fetchAndParseAsJSON(url, {
                body: JSON.stringify({
                ...
                headers,
                method: "PATCH",
            }, "renameFile"), "renameFile", logger);
            const content = fetchResponse.content;
            if (!content?.id) {
                throw new NonRetryableError("ODSP RenameFile call returned no item ID (for empty file)", OdspErrorTypes.incorrectServerResponse, { driverVersion });
            }
            event.end({
                ...fetchResponse.propsToLog,
            });
...
export async function createNewFluidFileFromSummary(getAuthHeader, newFileInfo, logger, createNewSummary, epochTracker, forceAccessTokenViaAuthorizationHeader) {
    ...
    return createNewFluidContainerCore({
        ...
        validateResponseCallback: (content) => {
            if (!content?.itemId) {
                throw new NonRetryableError("ODSP CreateFile call returned no item ID", OdspErrorTypes.incorrectServerResponse, { driverVersion });
            }
        },
    });
}
