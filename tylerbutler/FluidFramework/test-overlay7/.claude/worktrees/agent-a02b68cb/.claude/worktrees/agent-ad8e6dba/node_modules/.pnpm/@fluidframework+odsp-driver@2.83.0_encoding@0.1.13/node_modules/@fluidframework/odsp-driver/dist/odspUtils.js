...
const internal_3 = require("@fluidframework/odsp-doclib-utils/internal");
...
async function getWithRetryForTokenRefresh(get) {
    return get({ refresh: false }).catch(async (error) => {
        ...
        switch (error.errorType) {
            ...
            case internal_4.OdspErrorTypes.authorizationError: {
                const authError = error;
                return get({ ...options, claims: authError.claims, tenantId: authError.tenantId });
            }
            case internal_4.OdspErrorTypes.incorrectServerResponse: // some error on the wire, retry once
            case internal_4.OdspErrorTypes.fetchTokenError: {
...
async function fetchHelper(requestInfo, requestInit) {
    ...
    return fetch(requestInfo, requestInit).then(async (fetchResponse) => {
        ...
        if (!response) {
            ...
            "No response from ODSP fetch call", internal_4.OdspErrorTypes.incorrectServerResponse, { driverVersion: packageVersion_js_1.pkgVersion });
        }
        if (!response.ok || response.status < 200 || response.status >= 300) {
            (0, internal_3.throwOdspNetworkError)(
        ...
        const taggedErrorMessage = {
            ...
            tag: internal_5.TelemetryDataTag.UserData,
        };
        ...
        if (error.name === "AbortError") {
            throw new internal_2.RetryableError("Fetch Timeout (AbortError)", internal_4.OdspErrorTypes.fetchTimeout, {
                driverVersion: packageVersion_js_1.pkgVersion,
            });
        }
        // TCP/IP timeout
        if (redactedErrorText.includes("ETIMEDOUT")) {
            throw new internal_2.RetryableError("Fetch Timeout (ETIMEDOUT)", internal_4.OdspErrorTypes.fetchTimeout, {
                driverVersion: packageVersion_js_1.pkgVersion,
            });
        }
        // eslint-disable-next-line unicorn/prefer-ternary
        if (online === internal_2.OnlineStatus.Offline) {
            throw new internal_2.RetryableError(
            // pre-0.58 error message prefix: Offline
            `ODSP fetch failure (Offline): ${redactedErrorText}`, internal_4.OdspErrorTypes.offlineError, {
                driverVersion: packageVersion_js_1.pkgVersion,
            ...
            `ODSP fetch failure: ${redactedErrorText}`, internal_4.OdspErrorTypes.fetchFailure, {
                driverVersion: packageVersion_js_1.pkgVersion,
                rawErrorMessage: taggedErrorMessage,
            });
        }
 ...
 */
async function fetchArray(requestInfo, requestInit) {
    ...
    try {
        ...
        (0, internal_3.throwOdspNetworkError)("Error while parsing fetch response", internal_3.fetchIncorrectResponse, content, // response
 ...
 * @param requestInit - fetch requestInit
 */
async function fetchAndParseAsJSONHelper(requestInfo, requestInit) {
    const { content, headers, propsToLog, duration } = await fetchHelper(requestInfo, requestInit);
    let text;
    try {
        ...
        "Error while parsing fetch response", internal_3.fetchIncorrectResponse, content, // response
        text, propsToLog);
    }
...
exports.createOdspLogger = createOdspLogger;
/**
 ...
 */
function toInstrumentedOdspStorageTokenFetcher(logger, resolvedUrlParts, tokenFetcher) {
 ...
 */
function toInstrumentedOdspTokenFetcher(logger, resolvedUrlParts, tokenFetcher, throwOnNullToken, returnPlainToken) {
    return async (options, name, alwaysRecordTokenFetchTelemetry = false) => {
        ...
        // (fluid-fetcher)
        return internal_5.PerformanceEvent.timedExecAsync(logger, {
            ...
            const returnVal = returnPlainToken
                ? (0, internal_4.tokenFromResponse)(tokenResponse)
                : (0, internal_4.authHeaderFromTokenResponse)(tokenResponse);
            ...
            if (alwaysRecordTokenFetchTelemetry || event.duration >= 32) {
                event.end({
                    fromCache: (0, internal_4.isTokenFromCache)(tokenResponse),
            ...
            if (returnVal === null && throwOnNullToken) {
                ...
                `The Host-provided token fetcher returned null`, internal_4.OdspErrorTypes.fetchTokenError, { method: name, driverVersion: packageVersion_js_1.pkgVersion });
            }
            return returnVal;
        }, (error) => {
            ...
            const tokenError = (0, internal_5.wrapError)(error, (errorMessage) => new internal_2.NetworkErrorBasic(`The Host-provided token fetcher threw an error`, internal_4.OdspErrorTypes.fetchTokenError, typeof rawCanRetry === "boolean" ? rawCanRetry : false /* canRetry */, { method: name, errorMessage, driverVersion: packageVersion_js_1.pkgVersion }));
            throw tokenError;
        }), { cancel: "generic" });
    };
}
 ...
 */
function buildOdspShareLinkReqParams(shareLinkType) {
    if (!shareLinkType) {
        return;
    }
