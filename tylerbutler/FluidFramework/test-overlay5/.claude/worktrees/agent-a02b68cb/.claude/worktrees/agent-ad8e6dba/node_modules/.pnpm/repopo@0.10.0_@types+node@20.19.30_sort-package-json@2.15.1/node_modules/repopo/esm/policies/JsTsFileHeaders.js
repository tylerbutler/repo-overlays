...
import { defineFileHeaderPolicy, } from "../policyDefiners/defineFileHeaderPolicy.js";
const jsTsReplacer = (prevContent, { headerText }) => {
    // prepend header to existing content
    const separator = prevContent.startsWith("\r") || prevContent.startsWith("\n")
        ? newline
        : newline + newline;
    const newContent = `/*!${newline} * ${headerText.replace(newline, `${newline} * `)}${newline} */${separator}${prevContent}`;
    return newContent;
};
/**
...
export const JsTsFileHeaders = defineFileHeaderPolicy({
    ...
    description: "Ensures JavaScript and TypeScript source files have the configured header comment.",
    config: {
        match: /(^|\/)[^/]+\.[mc]?[jt]sx?$/i,
...
//# sourceMappingURL=JsTsFileHeaders.js.map