...
})(SnapshotFormatSupportType || (SnapshotFormatSupportType = {}));
/**
    ...
    return getWithRetryForTokenRefresh(async (tokenFetchOptions) => {
        ...
        return PerformanceEvent.timedExecAsync(logger, perfEvent, async (event) => {
            ...
            try {
                switch (contentTypeToRead) {
                    case "application/json": {
                        ...
                        [text, receiveContentTime] = await measureP(async () => odspResponse.content
                            ...
                            .then((res) => {
                            if (res.length === 0) {
                                throwOdspNetworkError("Response from browser is empty", fetchIncorrectResponse, odspResponse.content, // response
                    ...
                    case "application/ms-fluid": {
                        let content;
                        [content, receiveContentTime] = await measureP(async () => odspResponse.content
                            .arrayBuffer()
                            .then((res) => {
                            if (res.byteLength === 0) {
                                throwOdspNetworkError("Response from browser is empty", fetchIncorrectResponse, odspResponse.content, // response
                        ...
                        if (snapshotContents.snapshotTree.trees === undefined ||
                            ...
                            throw new NonRetryableError("Returned odsp snapshot is malformed. No trees or blobs!", OdspErrorTypes.incorrectServerResponse, propsToLog);
                        }
                    ...
                    default: {
                        throw new NonRetryableError("Unknown snapshot content type", OdspErrorTypes.incorrectServerResponse, propsToLog);
                    }
                }
            }
            catch (error) {
                ...
                const enhancedError = wrapError(error, (errorMessage) => new NonRetryableError(`Error parsing snapshot response: ${errorMessage}`, OdspErrorTypes.genericError, propsToLog));
                throw enhancedError;
            }
...
export function getTreeStats(snapshot) {
    const stats = {
        trees: 0,
        leafTrees: 0,
        blobNodes: 0,
...
export function validateBlobsAndTrees(snapshot) {
    ...
    assert(snapshot.blobs !== undefined, 0x201 /* "Returned odsp snapshot is malformed. No blobs!" */);
}
 ...
 */
export const downloadSnapshot = mockify(async (odspResolvedUrl, getAuthHeader, tokenFetchOptions, loadingGroupIds, snapshotOptions, snapshotFormatFetchType, controller, epochTracker, scenarioName) => {
    // back-compat: This block to be removed with #8784 when we only consume/consider odsp resolvers that are >= 0.51
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-explicit-any
    const sharingLinkToRedeem = odspResolvedUrl.sharingLinkToRedeem;
    if (sharingLinkToRedeem) {
