 ...
 */
function buildWorkspacePackageMap(root) {
    // Check cache first
 ...
 */
function findWorkspacePackage(depName, root) {
...
export const NoPrivateWorkspaceDependencies = definePackagePolicy({
    ...
    description: "Prevents publishable packages from depending on private workspace packages that won't be available on npm.",
    ...
    handler: function* (json, { file, root, config }) {
        ...
        if (privateWorkspaceDeps.length > 0) {
            errorMessages.push(`Publishable package has workspace dependencies on private packages that won't be available on npm:\n\t${privateWorkspaceDeps.join("\n\t")}`);
        }
        if (unresolvableDeps.length > 0) {
            errorMessages.push(`Could not resolve workspace dependencies (verify they exist):\n\t${unresolvableDeps.join("\n\t")}`);
        }
...
//# sourceMappingURL=NoPrivateWorkspaceDependencies.js.map