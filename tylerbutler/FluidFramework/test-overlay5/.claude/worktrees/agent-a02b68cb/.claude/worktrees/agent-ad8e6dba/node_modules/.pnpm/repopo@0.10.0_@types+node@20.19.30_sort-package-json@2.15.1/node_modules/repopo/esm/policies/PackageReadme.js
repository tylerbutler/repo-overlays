 ...
 */
function createReadmeContent(packageName, requiredContent) {
 ...
 */
async function handleMissingReadme(readmePath, packageName, file, resolve, requiredContent) {
    if (resolve) {
        try {
            await writeFile(readmePath, createReadmeContent(packageName, requiredContent));
            return {
                name: "PackageReadme",
                file,
                resolved: true,
 ...
 */
function validateTitle(readmeContent, packageName) {
 ...
 */
async function handleRequiredContent(readmePath, readmeContent, requiredContent, resolve) {
 ...
 */
async function validateExistingReadme(ctx) {
    const errors = [];
    let wasFixed = false;
    ...
    if (ctx.requiredContent) {
        const contentResult = await handleRequiredContent(ctx.readmePath, ctx.readmeContent, ctx.requiredContent, ctx.resolve);
    ...
    return {
        name: "PackageReadme",
        file: ctx.file,
        autoFixable: ctx.requiredContent !== undefined &&
...
export const PackageReadme = definePackagePolicy({
    name: "PackageReadme",
    description: "Ensures each package has a README.md file with proper title and required content.",
    handler: async (json, { file, resolve, config }) => {
        ...
        if (!existsSync(readmePath)) {
            return await handleMissingReadme(readmePath, packageName, file, resolve, requiredContent);
        }
...
//# sourceMappingURL=PackageReadme.js.map