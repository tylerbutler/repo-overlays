 ...
 */
class OdspDelayLoadedDeltaStream {
    /**
    ...
    annotateConnectionError(error, failedConnectionStep, separateTokenRequest) {
        return (0, internal_5.normalizeError)(error, {
            props: {
    ...
    async connectToDeltaStream(client) {
        (0, internal_1.assert)(this.currentConnection === undefined, 0x4ad /* Should not be called when connection is already present! */);
        ...
        return (0, odspUtils_js_1.getWithRetryForTokenRefresh)(async (options) => {
            ...
            if (finalWebsocketToken === null) {
                throw this.annotateConnectionError(new internal_2.NonRetryableError("Websocket token is null", internal_4.OdspErrorTypes.fetchTokenError, {
            ...
            try {
                const connection = await this.createDeltaConnection(websocketEndpoint.tenantId, websocketEndpoint.id, finalWebsocketToken, client, websocketEndpoint.deltaStreamSocketUrl, connectionId);
                ...
                connection.once("disconnect", (error) => {
                    ...
                    if (typeof error === "object" &&
                        error !== null &&
                        error.errorType === internal_4.OdspErrorTypes.authorizationError) {
                    ...
                    (0, internal_1.assert)(connection.disposed, 0x4ae /* Connection should be disposed by now */);
                ...
                if (error &&
                    typeof error === "object" &&
                    (error.scenarioName === "connect_document_error" ||
                        error.scenarioName === undefined)) {
                    this.clearJoinSessionTimer();
                    this.cache.sessionJoinCache.remove(this.joinSessionKey);
                }
    ...
    async joinSession(requestSocketToken, options, isRefreshingJoinSession, clientId, displayName) {
        ...
        if (isRefreshingJoinSession &&
            (this.currentConnection === undefined ||
                (clientId !== undefined && this.currentConnection.clientId !== clientId))) {
            this.clearJoinSessionTimer();
            throw new internal_2.NonRetryableError("JoinSessionRefreshTimerNotCancelled", internal_4.OdspErrorTypes.genericError, {
                driverVersion: packageVersion_js_1.pkgVersion,
                ...
                for (const code of error.facetCodes) {
                    switch (code) {
                        ...
                        case "sessionForbiddenOnInvisibleMinorVersion": {
                            ...
                            throw new internal_2.DeltaStreamConnectionForbiddenError(`Storage-only due to ${code}`, { driverVersion: packageVersion_js_1.pkgVersion }, code);
                        }
                        default: {
                            continue;
                        }
    ...
    async joinSessionCore(requestSocketToken, options, isRefreshingJoinSession, displayName) {
        ...
        const getResponseAndRefreshAfterDeltaMs = async () => {
            ...
            return {
                ..._response,
                refreshAfterDeltaMs: this.calculateJoinSessionRefreshDelta(_response.entryTime, _response.joinSessionResponse.refreshSessionDurationSeconds),
            };
        };
     ...
     */
    async createDeltaConnection(tenantId, documentId, token, client, webSocketUrl, connectionId) {
        const startTime = (0, client_utils_1.performanceNow)();
        const connection = await odspDocumentDeltaConnection_js_1.OdspDocumentDeltaConnection.create(tenantId, documentId, token, client, webSocketUrl, this.mc.logger, 60000, this.epochTracker, this.socketReferenceKeyPrefix, connectionId);
        const duration = (0, client_utils_1.performanceNow)() - startTime;
...
exports.OdspDelayLoadedDeltaStream = OdspDelayLoadedDeltaStream;
